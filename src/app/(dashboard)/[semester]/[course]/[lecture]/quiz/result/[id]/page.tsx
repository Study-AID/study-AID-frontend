...
export default function QuizResultPage() {
  ...
  return <QuizResultComponent quizId={quizId} />;
}

function QuizResultComponent({ quizId }: { quizId: string }) {
    ...
    return (
      <div className="flex min-h-screen flex-1 items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#757575]">퀴즈 결과를 불러오는 중...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !data || !quiz) {
    return (
      <div className="flex min-h-screen flex-1 items-center justify-center">
        <div className="text-center">
          <p className="mb-4 text-red-500">퀴즈 결과를 불러올 수 없습니다.</p>
  ...
  //       return question.answerIndices.every((index: number) => (userAnswer as number[]).includes(index))
  ...
  // 정답률 계산
  // const correctCount = quizItems.filter(isCorrect).length;
  // const totalCount = quizItems.length;
  // const correctPercentage =
  //   totalCount > 0 ? Math.round((correctCount / totalCount) * 100) : 0;
                  ...
                  <div className="mt-8 w-full border-t border-[#e6e6e6] pt-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium">
                        {quizItems.reduce((sum, item) => sum + item.points!, 0)}
                        점
                      </span>
                    </div>
                  </div>
